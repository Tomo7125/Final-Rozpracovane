...
public class Start {
    ...
    public boolean loginControl(String email, String password) {
        // V query si vytvorÌm dotaz na DB kolko riadkov obsahuje dan˝ email a heslo ( je potrebnÈ heslo posielaù uû zaöifrovanÈ )
        ...
        try (PreparedStatement statement = databaseConnection.prepareStatement(query)) {
            ...
            if (resultSet.next()) {
                ...
                //Do count sa mi uloûilo ËÌslo z prveho columu resultsetu , n·sledne niûöie skontrolujem Ëi je count veËöÌ ako 0
                // ak je veËöÌ tak v databaze je email a heslo ktore som zadal do loginu -> loginsuccessful sa zmenÌ na true
    ...
    //Metoda ktora mi bude vraciaù celu zloûku uûivatela podla emailu
    public User getUserByEmail(String email) {
        ...
        try (PreparedStatement statement = databaseConnection.prepareStatement(query)) {
            ...
            ResultSet resultSet = statement.executeQuery();
            if (resultSet.next()) {
                //Pouklad·m si do premenn˝ch ˙daje ktore som si vyûiadal podla n·zvov stlpcov
                String name = resultSet.getString("first_name");
                String lastName = resultSet.getString("last_name");
                String userEmail = resultSet.getString("email");
                int score = resultSet.getInt("score");
                boolean isAdmin = resultSet.getBoolean("isAdmin");
                //VytvorÌm si Usera a nasetujem mu premenne
